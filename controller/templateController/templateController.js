...
export const getAllCaseType = async (req, res, next) => {
  ...
  export const getTemplateByCaseType = async (req, res, next) => {
    try {
        ...
        const {deltaForm,faqs,customFields,imageUrl,videoUrl,summary} = template // Replace "deltaForm" with the actual field name in your document
    
        // Return the deltaForm as a response
        res.status(200).json({ deltaForm,faqs,customFields,imageUrl,videoUrl,summary});
    } catch (error) {
